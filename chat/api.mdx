...
Pass in a callback function that is called when the unread message count changes. If you pass in `null` as the callback function, the existing callback is removed.
```javascript
